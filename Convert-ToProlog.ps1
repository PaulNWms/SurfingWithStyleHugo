...
$response = Invoke-WebRequest -Headers $headers -uri "http://localhost:5984/guitar/_design/prolog/_view/prolog"
...
#    $filename = Join-Path '.\content\practice\chords' "$file.md"
    $row.value | Out-File -Append -Encoding utf7 guitar.pl
    $row.value
  }
}